    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && !$disabled) {
        ...
        try {
            if ((int) $pdo->query('SELECT COUNT(*) FROM admins')->fetchColumn() !== 0) {
                ...
                $statement = $pdo->prepare("INSERT INTO admins (username, email, password_hash, role) VALUES (:username, :email, :password_hash, 'superadmin')");
                $statement->execute(['username' => $username, 'email' => $email, 'password_hash' => password_hash($password, PASSWORD_DEFAULT)]);
                $success = true;
                $disabled = true;
            }
        } finally {
...
<?php else: ?><?php if ($error): ?><p class="admin-message admin-error" role="alert"><?= h($error) ?></p><?php endif; ?><form class="admin-form" method="post"><input type="hidden" name="csrf_token" value="<?= h(csrf_token()) ?>"><label>Admin username<input name="username" maxlength="100" autocomplete="username" required></label><label>Email address<input type="email" name="email" maxlength="255" autocomplete="email" required></label><label>Admin password <span>(at least 12 characters)</span><input type="password" name="password" minlength="12" autocomplete="new-password" required></label><label>Confirm password<input type="password" name="password_confirmation" minlength="12" autocomplete="new-password" required></label><button type="submit">CREATE ADMIN</button></form><?php endif; ?></main></body></html>
