...
<?php $editingImageUrl = safe_media_url($editing['image'] ?? null); ?>
<?php if ($editingImageUrl !== null): ?><div class="admin-current-image"><span>Current image (kept unless you select a replacement)</span><img class="admin-image-preview" src="<?= h(str_starts_with($editingImageUrl, 'http') || str_starts_with($editingImageUrl, '/') ? $editingImageUrl : '../' . $editingImageUrl) ?>" alt="Current post image"></div><?php endif; ?>
...
<div class="admin-actions"><button<?= $editing['id'] ? '' : ' class="admin-news-create"' ?> type="submit" name="action" value="save"><?= $editing['id'] ? 'SAVE CHANGES' : 'CREATE POST' ?></button><?php if ($editing['id']): ?><a href="news.php">CANCEL</a><?php endif; ?></div>
