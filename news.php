...
<?php foreach ($posts as $post): $titleFormat = news_format_style($post, 'title', false); $textFormat = news_format_style($post, 'text', true); ?><article class="news-post reveal"><time datetime="<?= news_h(date('c', strtotime($post['published_at']))) ?>"><?= news_h(date('F j, Y', strtotime($post['published_at']))) ?></time><h2<?= $titleFormat['custom_size'] ? ' class="news-custom-title-size"' : '' ?> style="<?= news_h($titleFormat['style']) ?>"><?= news_h($post['title']) ?></h2><?php if ($post['image']): ?><img src="<?= news_h($post['image']) ?>" alt=""><?php endif; ?><div class="news-content<?= $textFormat['custom_size'] ? ' news-custom-text-size' : '' ?>" style="<?= news_h($textFormat['style']) ?>"><?= news_rich_text_for_editor((string) $post['content']) ?></div></article><?php endforeach; ?>
